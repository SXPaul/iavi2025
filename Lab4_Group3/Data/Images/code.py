...
def project_cube_to_image(images, valid_indices, mtx, dist, rvecs, tvecs):
    ...
    #  6 cube faces: each face，and  they are defined as different colors to check whether the project is right
    cube_faces = [
        ([0, 1, 2, 3], (255, 0, 0)),   
        ([4, 5, 6, 7], (0, 255, 0)),    
        ([0, 1, 5, 4], (0, 0, 255)),   
    ...
    # Project cube,here give 3 images to test
    for i in range(min(3, len(valid_indices))):
        ...
        # Adjust 2D coordinates to match undistorted image
        cube_2d[:, 0] -= x_roi  
        cube_2d[:, 1] -= y_roi  
        
        # Fill each face with its color (transparency: 0.5 for better overlay)
        ...
        # Draw cube edges to highlight outline
